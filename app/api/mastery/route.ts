...
// Champion ID to name mapping (top champions for quick reference)
const CHAMPION_MAP: Record<number, string> = {
  ...
  777: 'Yone', 875: 'Sett', 876: 'Lillia', 887: 'Gwen', 888: 'Renata', 893: 'Aurora',
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const masteries = data.map((mastery: any) => ({
      championId: mastery.championId,
      championName: CHAMPION_MAP[mastery.championId] || `Champion${mastery.championId}`,
    ...
    return NextResponse.json(
      { error: 'Failed to fetch champion mastery', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
